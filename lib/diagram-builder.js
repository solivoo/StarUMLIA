...
/** Infiera capa DDD desde estereotipo/nombre si no viene layer */
...
function stereotypeOf(specOrModel) {
  if (!specOrModel) {
    return "";
  }
  const st = specOrModel.stereotype;
  if (typeof st === "string") {
    return st;
  }
  if (st && st.name) {
...
function inferLayer(spec) {
  if (spec && spec.layer) {
    return normalizeLayerName(spec.layer);
  }
  if (spec && spec.package) {
    return normalizeLayerName(spec.package);
  }
  const st = stereotypeOf(spec).toLowerCase();
  const name = String((spec && spec.name) || "");

  // Controllers / API antes que Adapter genérico
  if (/Controller$/i.test(name) || /controller|presentation|ui/.test(st)) {
    return "Presentation";
  }
  if (
  ...
  if (spec && spec.kind === "interface") {
    return "Interfaces";
  }
  return null;
}

function inferLayerFromModel(model) {
  if (!model) {
    return null;
  }
  const kind =
    model.getClassName && model.getClassName() === "UMLInterface"
      ? "interface"
      : "class";
  return inferLayer({
    name: model.name,
    stereotype: stereotypeOf(model),
    kind: kind
  });
}
...
function findPackageByName(parent, name) {
  if (!parent || !name) {
    return null;
  }
  function walk(el) {
    ...
    if (
      el.getClassName &&
      el.getClassName() === "UMLPackage" &&
      el.name === name
    ) {
      return el;
    }
    const kids = el.ownedElements || [];
...
function ensurePackage(parent, diagram, layerName, index, cache) {
  const name = normalizeLayerName(layerName) || layerName;
  if (cache[name]) {
    return cache[name];
  }

  let pkgModel = findPackageByName(parent, name);
  if (!pkgModel) {
    pkgModel = app.factory.createModel({
      id: "UMLPackage",
      parent: parent,
      modelInitializer: (m) => {
        m.name = name;
  ...
  let pkgView = getViewOf(diagram, pkgModel);
  const cols = 2;
  ...
  cache[name] = { model: pkgModel, view: pkgView, childCount: 0 };
  return cache[name];
}

/**
 * Mueve TODAS las clases/interfaces del diagrama a paquetes Domain/Application/…
 * según estereotipo. Solo crea paquetes que tengan al menos un elemento.
 *
 * Importante: NO basta con relocate + left/top. Si no se anida la vista
 * (containerView), el paquete opaco tapa las clases ("se quedan atrás").
 * Por eso recreamos la vista con createViewAndRelationships(..., containerView).
 */
function regroupDiagramByLayers(diagram) {
  if (!diagram) {
    return { moved: 0, packages: 0 };
  }
  ...
  function collect(views) {
    (views || []).forEach((v) => {
      if (v && v.model && isClassifierModel(v.model)) {
        classViews.push(v);
      }
      if (v && v.containedViews && v.containedViews.length) {
        collect(v.containedViews);
      }
    });
  }
  collect(diagram.ownedViews);
  ...
  const byLayer = {};
  classViews.forEach((v) => {
    const layer = inferLayerFromModel(v.model) || "Shared";
    if (!byLayer[layer]) {
      byLayer[layer] = [];
    }
    byLayer[layer].push(v);
  });

  const layers = Object.keys(byLayer).sort((a, b) => {
  ...
  // Posicionar y dimensionar paquetes ANTES de meter hijos
  layers.forEach((layer, idx) => {
    const pkg = packageCache[layer];
    if (!pkg || !pkg.view) {
      return;
    }
    const count = byLayer[layer].length;
    const localCols = Math.min(3, Math.max(1, count));
    const rows = Math.ceil(count / localCols);
    const w = Math.max(360, 50 + localCols * 300);
    const h = Math.max(220, 70 + rows * 200);
    const x = 40 + (idx % 2) * Math.max(w + 80, 720);
    const y = 40 + Math.floor(idx / 2) * (h + 100);
    try {
      app.engine.setProperty(pkg.view, "left", x);
      app.engine.setProperty(pkg.view, "top", y);
      app.engine.setProperty(pkg.view, "width", w);
      app.engine.setProperty(pkg.view, "height", h);
    } catch (err) {
      pkg.view.left = x;
      pkg.view.top = y;
      pkg.view.width = w;
  ...
  // Paquetes detrás (índice bajo): se dibujan primero
  bringPackagesBehind(diagram, packageCache);

  let moved = 0;
  layers.forEach((layer) => {
    const pkg = packageCache[layer];
    const views = byLayer[layer] || [];
    if (!pkg || !pkg.view || !pkg.model) {
      return;
    }
    const localCols = Math.min(3, Math.max(1, views.length));
    views.forEach((view, ci) => {
      const targetX = pkg.view.left + 30 + (ci % localCols) * 290;
      const targetY = pkg.view.top + 55 + Math.floor(ci / localCols) * 190;
      if (nestViewInsidePackage(view, pkg, targetX, targetY, editor)) {
        moved += 1;
        pkg.childCount = (pkg.childCount || 0) + 1;
      }
    });
  });

  applyRoundedLines(diagram);
  removeEmptyLayerPackages(diagram);
  try {
  ...
  return {
    moved: moved,
    packages: layers.length
  };
}

/**
 * Anida una vista de clase/interfaz DENTRO de un paquete.
 * Preferencia: recrear la vista con containerView (fiable en StarUML).
 */
function nestViewInsidePackage(view, pkg, x, y, editor) {
  if (!view || !view.model || !pkg || !pkg.view || !pkg.model) {
    return false;
  }
  const model = view.model;
  const diagram = (editor && editor.diagram) || app.diagrams.getCurrentDiagram();

  // Ya está bien anidada → solo reposicionar
  if (view.containerView === pkg.view) {
    try {
      if (editor && app.engine.moveViews) {
        const dx = x - (view.left || 0);
        const dy = y - (view.top || 0);
        if (dx !== 0 || dy !== 0) {
          app.engine.moveViews(editor, [view], dx, dy);
        }
      } else {
        app.engine.setProperty(view, "left", x);
        app.engine.setProperty(view, "top", y);
  ...
  // 1) Relocar modelo al paquete
  try {
    if (model._parent !== pkg.model && app.engine.relocate) {
      app.engine.relocate(model, pkg.model, "ownedElements");
    }
  } catch (err) {
    console.warn("[Kimi] relocate", model.name, err);
  }

  // 2) Intento oficial: cambiar contenedor moviendo
  if (editor && app.engine.moveViewsChangingContainer) {
    try {
      const dx = x - (view.left || 0);
      const dy = y - (view.top || 0);
      ...
      console.warn("[Kimi] moveViewsChangingContainer", model.name, err);
    }
  }

  // 3) Fallback fiable: borrar vista y recrearla dentro del paquete
  try {
    app.engine.deleteElements([], [view]);
  } catch (err) {
    console.warn("[Kimi] delete old view", model.name, err);
    return false;
  }

  try {
    let newView = null;
    if (editor && app.factory.createViewAndRelationships) {
      newView = app.factory.createViewAndRelationships(
        editor,
        x,
        y,
        model,
    ...
    if (!newView && diagram) {
      newView = placeClass(diagram, model, x, y, pkg.view);
    }
    if (newView) {
      try {
        app.engine.setProperty(newView, "suppressAttributes", false);
        app.engine.setProperty(newView, "suppressOperations", false);
        app.engine.setProperty(newView, "left", x);
        app.engine.setProperty(newView, "top", y);
      } catch (err) {
        /* ignore */
      }
      return true;
    ...
    console.warn("[Kimi] recreate view in package", model.name, err);
  }
  return false;
}

/** Paquetes al inicio de ownedViews = se dibujan detrás; clases encima */
function bringPackagesBehind(diagram, packageCache) {
  if (!diagram || !diagram.ownedViews) {
    return;
  }
  const pkgViews = Object.keys(packageCache)
    .map((k) => packageCache[k] && packageCache[k].view)
    .filter(Boolean);

  pkgViews.forEach((pv, i) => {
    try {
      // índice bajo = detrás
      if (app.engine.setProperty && diagram.ownedViews.indexOf(pv) > i) {
        const builder = app.repository.getOperationBuilder();
        builder.begin("reorder package behind");
        builder.fieldReorder(diagram, "ownedViews", pv, i);
        builder.end();
...
/** Elimina paquetes de capa DDD vacíos (sin clases/interfaces visibles dentro) */
function removeEmptyLayerPackages(diagram) {
  if (!diagram) {
    return 0;
  }
  const layerNames = new Set(LAYER_ORDER);
  let removed = 0;
  const packageViews = (diagram.ownedViews || []).filter((v) => {
    return (
      v &&
      v.model &&
      v.model.getClassName &&
      v.model.getClassName() === "UMLPackage" &&
      layerNames.has(v.model.name)
    );
  });

  packageViews.forEach((pv) => {
    const visuallyHas = (pv.containedViews || []).some(
      (cv) => cv && cv.model && isClassifierModel(cv.model)
    );
    if (!visuallyHas) {
      try {
        // Solo borrar la VISTA del paquete vacío; el modelo puede
        // conservarse si aún tiene elementos en el explorador.
        // Si el modelo tampoco tiene clasificadores, borrar ambos.
        const modelHas = (pv.model.ownedElements || []).some((e) =>
          isClassifierModel(e)
        );
        if (modelHas) {
          // Modelo tiene clases pero la vista está vacía → no borrar modelo;
          // borrar solo la caja vacía haría perder el contenedor. Mejor dejarla
          // y dejar que nest las meta dentro en un siguiente pass.
          return;
        }
        app.engine.deleteElements([pv.model], [pv]);
        removed += 1;
      } catch (err) {
        console.warn("[Kimi] remove empty package", pv.model.name, err);
      }
    }
  });
  return removed;
...
function isLayerPackageName(name) {
  if (!name) {
    return false;
  }
  const n = normalizeLayerName(name) || String(name);
  return LAYER_ORDER.indexOf(n) >= 0;
}
...
function connect(diagram, fromView, toView, assoc) {
  ...
  // No conectar paquetes entre sí (Kimi a veces inventa Domain→Application)
  const fromCn = fromView.model && fromView.model.getClassName && fromView.model.getClassName();
  const toCn = toView.model && toView.model.getClassName && toView.model.getClassName();
  if (fromCn === "UMLPackage" || toCn === "UMLPackage") {
    console.warn("[Kimi] ignorando relación entre paquetes:", assoc && assoc.from, "→", assoc && assoc.to);
    return null;
  }
  if (isLayerPackageName(assoc && assoc.from) || isLayerPackageName(assoc && assoc.to)) {
    console.warn("[Kimi] ignorando relación con nombre de capa:", assoc && assoc.from, "→", assoc && assoc.to);
    return null;
  }
...
function hasDddStereotype(c) {
  if (!c) {
    return false;
  }
  if (isInterfaceSpec(c)) {
    return true;
  }
  const st = stereotypeOf(c).toLowerCase();
  return /aggregateroot|entity|valueobject|repository|domainservice|applicationservice|dto|port|adapter|usecase|command|query|factory|controller|handler/.test(
    st
  );
}
...
function shouldGroupByLayers(spec) {
  ...
  const list = []
    .concat(spec.classes || [])
    .concat(spec.addClasses || [])
    .concat(spec.updateClasses || []);
  if (list.some((c) => c && (c.layer || c.package))) {
    return true;
  }
  // Si el diagrama huele a DDD, agrupar siempre (no depender de que Kimi mande layer)
  return list.filter(hasDddStereotype).length >= 2;
}
...
function applyCreate(spec) {
  ...
  // Con capas, layout dagre puede sacar clases de paquetes; reagrupar + redondear
  if (useLayers) {
    regroupDiagramByLayers(diagram);
  } else {
    autoLayout(diagram);
  }
  app.diagrams.repaint();
...
function applyPatch(spec) {
  ...
  const wantLayers = hasPackages || shouldGroupByLayers(spec);
  // Reagrupar TODO el diagrama (no solo lo nuevo) para Domain/Application/Infrastructure/…
  if (wantLayers) {
    regroupDiagramByLayers(diagram);
