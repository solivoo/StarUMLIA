...
function autoLayout(diagram) {
  ...
  const roundRect =
    (type.EdgeView && type.EdgeView.LS_ROUNDRECT != null)
      ...
      : 2;
  const separations = { node: 60, edge: 40, rank: 90 };

  try {
    app.engine.layoutDiagram(editor, diagram, dir, separations, roundRect);
    app.diagrams.repaint();
    return true;
  } catch (err) {
    console.warn("[Kimi] autoLayout falló:", err);
    // Fallback: al menos forzar rounded rectilinear en las aristas
    try {
      (diagram.ownedViews || []).forEach((v) => {
        if (v && v.getClassName && /EdgeView$|View$/.test(v.getClassName()) && "lineStyle" in v) {
          app.engine.setProperty(v, "lineStyle", roundRect);
        }
      });
      app.diagrams.repaint();
    } catch (err2) {
...
function applyRoundedLines(diagram) {
  ...
  const roundRect =
    (type.EdgeView && type.EdgeView.LS_ROUNDRECT != null)
      ...
      : 2;
  (diagram.ownedViews || []).forEach((v) => {
    if (v && typeof v.lineStyle !== "undefined") {
      try {
        app.engine.setProperty(v, "lineStyle", roundRect);
      } catch (err) {
        v.lineStyle = roundRect;
      }
    }
  });
}
