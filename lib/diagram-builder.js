 ...
 * según estereotipo. Sirve para arreglar diagramas ya generados.
 */
function regroupDiagramByLayers(diagram) {
  ...
  // Recolectar vistas de clasificadores (también anidadas)
  const classViews = [];
  ...
  // Pre-crear paquetes para capas detectadas + las 4 principales DDD
  const needed = new Set(["Domain", "Application", "Infrastructure", "Presentation"]);
  ...
  let moved = 0;
  classViews.forEach((view) => {
    const model = view.model;
    const layer = inferLayerFromModel(model) || "Shared";
    const pkg = packageCache[layer];
    if (!pkg || !pkg.model) {
      return;
    }

    // Relocar modelo al paquete
    try {
    ...
    // Anidar vista en el paquete
    const ci = pkg.childCount || 0;
    const localCols = 2;
    const x = (pkg.view ? pkg.view.left : 40) + 30 + (ci % localCols) * 300;
    const y = (pkg.view ? pkg.view.top : 40) + 50 + Math.floor(ci / localCols) * 200;
    pkg.childCount = ci + 1;
    ...
    try {
      ...
      app.engine.setProperty(view, "top", y);
      if (pkg.view) {
        // quitar de contenedor anterior
        if (view.containerView && view.containerView !== pkg.view) {
          try {
            const old = view.containerView;
            if (old.containedViews && old.containedViews.includes(view)) {
              app.engine.setProperty(
                old,
                "containedViews",
                old.containedViews.filter((v) => v !== view)
              );
            }
          } catch (err) {
            /* ignore */
        ...
        app.engine.setProperty(view, "containerView", pkg.view);
        if (pkg.view.containedViews && !pkg.view.containedViews.includes(view)) {
          try {
            app.engine.setProperty(
              pkg.view,
              "containedViews",
              (pkg.view.containedViews || []).concat([view])
            );
          } catch (err) {
            /* ignore */
          }
      ...
      console.warn("[Kimi] nest view", model.name, err);
    }
  });

  // Ajustar tamaño de paquetes según hijos
  Object.keys(packageCache).forEach((name) => {
    const pkg = packageCache[name];
    if (!pkg.view) {
      return;
    }
    const rows = Math.max(1, Math.ceil((pkg.childCount || 1) / 2));
    const h = Math.max(220, 80 + rows * 210);
    const w = Math.max(640, (pkg.childCount > 2 ? 900 : 640));
    try {
  ...
  return {
    moved: moved,
    packages: Object.keys(packageCache).length
  };
}
